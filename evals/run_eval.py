...
# ---------------------------------------------------------------------------
# Fake similarity (offline eval — no real embeddings)
#
# We use a simple heuristic similarity based on word-overlap for the offline harness.
# The eval's value is in showing the *structural* difference between the two cases
# (surface-close vs surface-distant), not in the exact cosine values. A live eval
# with real embeddings would use actual cosine scores from the embedding model.
# ---------------------------------------------------------------------------


def _word_overlap_similarity(a: str, b: str) -> float:
    """Jaccard word-overlap as a proxy for cosine similarity in the offline eval."""
...
async def _run_gate(case: EvalCase, gate: IntentGate) -> str:
    similarity = _word_overlap_similarity(case.cached_question, case.new_question)
...
async def run_eval(verifier_score: float = 0.0) -> list[EvalResult]:
    ...
    for case in EVAL_CASES:
        ...
        baseline_verdict = _cosine_only_verdict(
            case, similarity, settings.cosine_baseline_threshold
        )
        results.append(
            EvalResult(
                case_id=case.id,
...
def _print_report(results: list[EvalResult]) -> None:
    ...
    print("-" * 80)
    for r in results:
        ok = "✓" if r.gate_correct else "✗ WRONG"
        print(
